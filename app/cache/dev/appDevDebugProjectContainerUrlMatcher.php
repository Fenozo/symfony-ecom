...
class appDevDebugProjectContainerUrlMatcher extends Symfony\Bundle\FrameworkBundle\Routing\RedirectableUrlMatcher
{
    ...
    public function match($rawPathinfo)
    {
        ...
        // utilisateur_utilisateur_default_index
        if ('/utilisateur' === rtrim($pathinfo, '/')) {
            if ('/' === substr($pathinfo, -1)) {
                // no-op
            } elseif (!in_array($this->context->getMethod(), array('HEAD', 'GET'))) {
                goto not_utilisateur_utilisateur_default_index;
            } else {
                return $this->redirect($rawPathinfo.'/', 'utilisateur_utilisateur_default_index');
            }

            return array (  '_controller' => 'Utilisateur\\UtilisateurBundle\\Controller\\DefaultController::indexAction',  '_route' => 'utilisateur_utilisateur_default_index',);
        }
        not_utilisateur_utilisateur_default_index:

        // acme_ecommerce_default_index
        if ('/ecommerce' === rtrim($pathinfo, '/')) {
            if ('/' === substr($pathinfo, -1)) {
                // no-op
            } elseif (!in_array($this->context->getMethod(), array('HEAD', 'GET'))) {
                goto not_acme_ecommerce_default_index;
            } else {
                return $this->redirect($rawPathinfo.'/', 'acme_ecommerce_default_index');
            }

            return array (  '_controller' => 'Acme\\EcommerceBundle\\Controller\\DefaultController::indexAction',  '_route' => 'acme_ecommerce_default_index',);
        }
        not_acme_ecommerce_default_index:
